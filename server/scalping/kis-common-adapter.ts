...
export function adaptKisExecution(event: KisExecutionEvent): NormalizedTrade {
  return NormalizedTradeSchema.parse({
    ...
    ...(event.marketCountry === "US" ? { sessionFeed: event.usFeed ?? "standard" } : {}),
    sessionDate: expandedDate(event.sessionDate),
    ...
    cumulativeTradingAmount: event.accumulatedTradingAmount,
    ...(event.executionStrength === undefined ? {} : { executionStrength: event.executionStrength }),
    ...(event.executionClassCode === undefined ? {} : { executionClassCode: event.executionClassCode }),
    bestBidPrice: event.bidPrice1,
    bestAskPrice: event.askPrice1,
  });
}
