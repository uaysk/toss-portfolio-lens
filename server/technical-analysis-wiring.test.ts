...
const coreRuntimeSource = readFileSync(
  new URL("./runtime/core-services.ts", import.meta.url),
  "utf8",
);
const assemblySource = runtimeSource + coreRuntimeSource;
...
describe("technical analysis server wiring", () => {
  it("공통 HTTP/MCP tool registry와 실제 TechnicalAnalysisService assembly를 연결한다", () => {
    ...
    expect(coreRuntimeSource).toContain("new TechnicalAnalysisService(");
    expect(coreRuntimeSource).toContain("technicalAnalysis: input.services.technicalAnalysisService");
  ...
  it("typed 기술 신호 검증·백테스트 tool을 같은 TechnicalStrategyService assembly에 연결한다", () => {
    ...
    expect(coreRuntimeSource).toContain("new TechnicalStrategyService(");
    expect(coreRuntimeSource).toContain("technicalStrategies: input.services.technicalStrategyService");
  });

  it("거래 marker 조회를 세션 인증 GET endpoint에 조립한다", () => {
    expect(assemblySource).toContain("new TechnicalTradeMarkerService(");
    expect(runtimeSource).toContain("technicalTradeMarkerService,");
