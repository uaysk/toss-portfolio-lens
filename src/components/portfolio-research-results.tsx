      ...
      <div className="grid gap-3 xl:grid-cols-[1.25fr_0.75fr]">
        <div className="rounded-[20px] bg-card p-3" role="group" aria-label="후보 변동성과 수익률 Pareto 산점도 탐색기">
          ...
          <div className="h-[330px]">
            <ResponsiveContainer width="100%" height="100%"><ScatterChart margin={{ top: 12, right: 10, bottom: 20, left: 0 }}><CartesianGrid stroke="hsl(var(--chart-grid))" /><XAxis type="number" dataKey="risk" name="변동성" unit="%" tickLine={false} axisLine={false} tick={{ fontSize: 9, fill: "hsl(var(--muted-foreground))" }} /><YAxis type="number" dataKey="reward" name="수익률" unit="%" width={48} tickLine={false} axisLine={false} tick={{ fontSize: 9, fill: "hsl(var(--muted-foreground))" }} /><ZAxis type="number" dataKey="robust" range={[28, 100]} /><Tooltip cursor={{ stroke: CHART_COLORS.cursor }} formatter={(value, name) => name === "변동성" || name === "수익률" ? formatPercent(Number(value), true) : Number(value).toFixed(3)} contentStyle={chartTooltipStyle} /><Scatter name="후보" data={regularPoints} fill={CHART_COLORS.neutral} fillOpacity={0.35} cursor="pointer" onClick={selectChartPoint} /><Scatter name="Pareto" data={paretoPoints} fill={CHART_COLORS.primary} cursor="pointer" onClick={selectChartPoint} /><Scatter name="선택" data={selectedPoints} fill={CHART_COLORS.positive} stroke="hsl(var(--chart-surface))" strokeWidth={2} cursor="pointer" onClick={selectChartPoint} /></ScatterChart></ResponsiveContainer>
          </div>
        </div>
