...
function SymbolLabel({ symbol, theme }: { symbol: string; theme: Theme }) {
  return <span className="inline-flex items-center gap-2"><StockSwatch symbol={symbol} theme={theme} />{symbol}</span>;
}

function DiversifyingResult({ result, theme }: { result: unknown; theme: Theme }) {
  const candidates = array(record(result).candidates).map(record);
  return <div className="overflow-x-auto rounded-[20px] bg-card p-3"><table className="w-full min-w-[760px] text-left text-xs"><thead><tr className="text-muted-foreground"><th className="p-3">후보</th><th className="p-3">상관</th><th className="p-3">하락장 상관</th><th className="p-3">Beta</th><th className="p-3">변동성 감소</th><th className="p-3">혼합 CAGR</th><th className="p-3">혼합 MDD</th></tr></thead><tbody>{candidates.map((candidate) => { const effect = record(candidate.expected_variance_effect); const mixed = record(candidate.mixed_portfolio_metrics); const symbol = String(candidate.symbol); return <tr key={symbol} className="border-t border-border"><td className="p-3 font-black"><SymbolLabel symbol={symbol} theme={theme} /></td><td className="p-3">{ratio(candidate.correlation)}</td><td className="p-3">{ratio(candidate.down_market_correlation)}</td><td className="p-3">{ratio(candidate.beta)}</td><td className="p-3">{decimalPercent(effect.volatility_reduction)}</td><td className="p-3">{decimalPercent(mixed.cagr)}</td><td className="p-3">{decimalPercent(mixed.max_drawdown)}</td></tr>; })}</tbody></table>{!candidates.length ? <p className="p-4 text-xs text-muted-foreground">상관 기준을 통과한 후보가 없습니다.</p> : null}</div>;
}
...
function ContributionResult({ result, theme }: { result: unknown; theme: Theme }) {
  const data = record(result);
