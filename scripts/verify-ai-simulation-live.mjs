...
assert(status.schemaVersion === "ai-paper-simulation/v6", "시뮬레이션 status 계약이 v6가 아닙니다.");
...
const start = await json(await fetch(`${baseUrl}/api/portfolio/simulation/runs`, {
  ...
  body: JSON.stringify({
    ...
    riskTolerance,
    costs: {
      commissionBpsPerSide: marketCountry === "KR" ? 1.5 : 10,
      taxBpsOnExit: marketCountry === "KR" ? 20 : 0,
...
try {
  ...
  assert(latest.snapshot.schemaVersion === "ai-paper-simulation/v6", "run snapshot 계약이 v6가 아닙니다.");
