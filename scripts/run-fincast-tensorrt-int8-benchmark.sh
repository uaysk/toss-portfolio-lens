...
if [[ $# -lt 1 || $# -gt 2 ]]; then
  echo "usage: run-fincast-tensorrt-int8-benchmark.sh <absolute-run-root> [canonical|router-int8]" >&2
  exit 2
fi
...
case "${VARIANT}" in
  canonical)
    ...
    ;;
  router-int8)
    ENGINE="${OUTPUT}/fincast-c60-b48-attempt2-router-linear-int8.plan"
    RESULT_SUFFIX=".router-int8"
    ;;
  *)
    echo "unknown TensorRT benchmark variant: ${VARIANT}" >&2
    exit 2
    ;;
esac
...
RESULT="${OUTPUT}/benchmark-result${RESULT_SUFFIX}.json"
TELEMETRY="${OUTPUT}/benchmark-gpu${RESULT_SUFFIX}.csv"
...
if [[ "${RUN_ROOT}" != /* ]] \
  ...
  echo "TensorRT INT8 benchmark preflight failed" >&2
  exit 2
fi
if [[ -e "${RESULT}" ]] || [[ -L "${OUTPUT}" ]]; then
  echo "TensorRT INT8 benchmark output already exists or is unsafe" >&2
  exit 2
fi
...
restore_production() {
  ...
  if [[ ${STOPPED_PRODUCTION} -eq 1 ]]; then
    ...
    event "production_restore" "tensorrt_benchmark_${health:-unavailable}"
    if [[ "${health}" != "healthy" ]]; then
  ...
  if [[ ${STOPPED_GPU_PEER} -eq 1 ]]; then
    if sudo -n systemctl start "${GPU_PEER_SERVICE}"; then
      event "gpu_peer_restore" "tensorrt_benchmark_${GPU_PEER_SERVICE}_active"
    else
      event "gpu_peer_restore" "tensorrt_benchmark_${GPU_PEER_SERVICE}_failed"
      exit 3
    fi
  fi
  exit "${exit_code}"
...
if [[ "$(docker inspect --format '{{.State.Status}} {{.State.Health.Status}}' "${PRODUCTION_CONTAINER}")" != "running healthy" ]] \
  ...
  echo "TensorRT benchmark production/GPU preflight failed" >&2
  exit 2
fi

event "int8_benchmark_preflight" "three_rounds_10_warmups_30_iterations_160w"
...
if [[ -n "$(nvidia-smi --query-compute-apps=pid --format=csv,noheader,nounits 2>/dev/null)" ]]; then
  event "int8_benchmark_rejected" "gpu_exclusivity_failed"
  exit 4
fi
...
event "int8_benchmark_start" "c60_b48_holdout32_${VARIANT}"
...
/usr/bin/time -v "${VENV}/bin/python" "${TRT_ROOT}/benchmark_int8.py" \
  ...
  --reference "${REFERENCE}" \
  --output "${RESULT}" \
  > "${OUTPUT}/benchmark${RESULT_SUFFIX}.log" \
  2> "${OUTPUT}/benchmark-resource${RESULT_SUFFIX}.log"
status=$?
event "int8_benchmark_finish" "c60_b48_${VARIANT}_exit_${status}"
exit "${status}"
