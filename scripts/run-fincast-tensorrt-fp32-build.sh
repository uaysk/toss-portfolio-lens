...
if [[ $# -ne 1 ]]; then
  echo "usage: run-fincast-tensorrt-fp32-build.sh <absolute-run-root>" >&2
  exit 2
fi
...
TIMING_CACHE="${OUTPUT}/timing-fp32.cache"
INSPECTOR="${OUTPUT}/inspector.fp32.json"
RESULT="${OUTPUT}/build-result.fp32.json"
EVENT_LOG="${TRT_ROOT}/events.jsonl"
TELEMETRY="${OUTPUT}/build-gpu.fp32.csv"
...
if [[ "${RUN_ROOT}" != /* ]] \
  ...
  || [[ ! -f "${ONNX}" ]] \
  || [[ ! -d "${SDK_ROOT}/lib" ]]; then
  echo "TensorRT FP32 build preflight failed" >&2
  exit 2
fi
if [[ -e "${ENGINE}" ]] \
  || [[ -e "${RESULT}" ]] \
  || [[ -e "${INSPECTOR}" ]] \
  || [[ -L "${OUTPUT}" ]]; then
  echo "TensorRT FP32 output already exists or is unsafe" >&2
  exit 2
fi
mkdir -p "${OUTPUT}"
...
restore_production() {
  ...
  if [[ ${STOPPED_PRODUCTION} -eq 1 ]]; then
    ...
    event "production_restore" "tensorrt_fp32_build_${health:-unavailable}"
    if [[ "${health}" != "healthy" ]]; then
  ...
  if [[ ${STOPPED_GPU_PEER} -eq 1 ]]; then
    if sudo -n systemctl start "${GPU_PEER_SERVICE}"; then
      event \
        "gpu_peer_restore" \
        "tensorrt_fp32_build_${GPU_PEER_SERVICE}_active"
    else
      event \
        "gpu_peer_restore" \
        "tensorrt_fp32_build_${GPU_PEER_SERVICE}_failed"
      exit 3
    fi
  fi
  exit "${exit_code}"
...
if [[ "$(docker inspect --format '{{.State.Status}} {{.State.Health.Status}}' "${PRODUCTION_CONTAINER}")" != "running healthy" ]] \
  ...
  echo "TensorRT FP32 build production/GPU preflight failed" >&2
  exit 2
fi

event \
  "fp32_build_preflight" \
  "tensorrt_8_6_1_cuda_12_2_cudnn_8_9_7_sm61_160w_c60_b48"
...
if [[ -n "$(nvidia-smi --query-compute-apps=pid --format=csv,noheader,nounits 2>/dev/null)" ]]; then
  event "fp32_build_rejected" "gpu_exclusivity_failed"
  exit 4
fi
...
event "fp32_build_start" "strict_fp32_obey_precision_constraints"
CUDA_HOME=/usr/local/cuda \
TENSORRT_ROOT="${SDK_ROOT}" \
LD_LIBRARY_PATH="${SDK_ROOT}/lib:/usr/local/cuda/lib64" \
PYTHONPATH="${TRT_ROOT}" \
/usr/bin/time -v "${VENV}/bin/python" "${TRT_ROOT}/build_fp32.py" \
  --onnx "${ONNX}" \
  --plugin "${PLUGIN}" \
  --engine "${ENGINE}" \
  ...
  --inspector "${INSPECTOR}" \
  --result "${RESULT}" \
  > "${OUTPUT}/build.fp32.log" \
  2> "${OUTPUT}/build-resource.fp32.log"
status=$?
event "fp32_build_finish" "c60_b48_exit_${status}"
exit "${status}"
