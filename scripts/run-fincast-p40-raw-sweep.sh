...
if [[ $# -ne 1 ]]; then
  echo "usage: run-fincast-p40-raw-sweep.sh <absolute-run-root>" >&2
  exit 2
fi

RUN_ROOT=$1
RESULT_PREFIX=${FINCAST_SWEEP_RESULT_PREFIX:-eager}
PRODUCTION_CONTAINER=toss-portfolio-lens-ai-worker-fincast-worker-1
GPU_PEER_SERVICE=llama-swap.service
SOURCE_ROOT="${RUN_ROOT}/source"
INPUT_ROOT="${RUN_ROOT}/inputs"
RESULT_ROOT="${RUN_ROOT}/results"
EVENT_LOG="${RUN_ROOT}/sweep-events.jsonl"
...
if [[ ! "${RESULT_PREFIX}" =~ ^[a-z0-9][a-z0-9-]{0,63}$ ]]; then
  echo "FINCAST_SWEEP_RESULT_PREFIX is invalid" >&2
  exit 2
fi
...
if [[ "${INITIAL_STATUS}" != "running" ]] || [[ "${INITIAL_HEALTH}" != "healthy" ]]; then
  echo "production FinCast must be running and healthy before the sweep" >&2
  exit 2
fi
if [[ "${INITIAL_GPU_PEER_STATUS}" != "active" ]]; then
  echo "${GPU_PEER_SERVICE} must be active before the sweep so its state can be restored" >&2
  exit 2
fi
...
wait_for_gpu_idle() {
  ...
  event "gpu_exclusivity_failed" "non_fincast_compute_process_remained_after_120s"
  printf '%s\n' "${processes}" >&2
  return 1
}
...
mkdir -p "${RESULT_ROOT}"
event "preflight" "power_limit_160w_image_and_cache_verified"
docker stop --time 30 "${PRODUCTION_CONTAINER}" >/dev/null
STOPPED_PRODUCTION=1
event "production_stop" "stopped_for_exclusive_fincast_measurement"
if ! sudo -n systemctl stop "${GPU_PEER_SERVICE}"; then
  event "gpu_peer_stop_failed" "${GPU_PEER_SERVICE}"
  exit 4
fi
STOPPED_GPU_PEER=1
event "gpu_peer_stop" "${GPU_PEER_SERVICE}_stopped_for_exclusive_measurement"
if ! wait_for_gpu_idle; then
  exit 4
fi
event "gpu_exclusive" "no_other_compute_process_before_sweep"

for cadence in 15 30 60; do
  for batch in 16 24 32 48 50; do
    output="${RESULT_ROOT}/${RESULT_PREFIX}-c${cadence}-b${batch}.json"
    name="fincast-raw-${RESULT_PREFIX}-c${cadence}-b${batch}-$(basename "${RUN_ROOT}")"
    if [[ -e "${output}" ]]; then
      event "candidate_skip" "${RESULT_PREFIX}_c${cadence}_b${batch}_output_exists"
      continue
    fi
    if ! wait_for_gpu_idle; then
      event "candidate_unavailable" "${RESULT_PREFIX}_c${cadence}_b${batch}_gpu_not_exclusive"
      continue
    fi
    event "candidate_start" "${RESULT_PREFIX}_c${cadence}_b${batch}"
    docker run --rm \
      --name "${name}" \
      --user 1000:1000 \
      --gpus all \
      ...
      --warmups 10 \
      --iterations 30 \
      >> "${RUN_ROOT}/sweep.log" 2>&1
    candidate_status=$?
    event "candidate_finish" "${RESULT_PREFIX}_c${cadence}_b${batch}_exit_${candidate_status}"
  done
done

event "sweep_finish" "all_${RESULT_PREFIX}_candidates_attempted"
