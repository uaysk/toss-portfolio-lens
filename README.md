...
- 초기 투자금, 월 정기 납입·인출, 리밸런싱 없음·월·분기·연 단위 조건을 지원합니다.
- KOSPI·KOSDAQ과 나스닥 100(QQQ)·S&P 500(SPY) 프록시를 비교할 수 있으며, 현금흐름 제거 성장, CAGR, 변동성, MDD, 샤프·소르티노, 연도별 수익률, 종목별 성과 기여도와 일간 수익률 상관관계를 표시합니다.
- 국내·해외 혼합 포트폴리오는 각 종목의 현지 통화 수정주가 수익률을 합성합니다. 과거 환율, 배당 현금흐름, 세금, 거래비용과 슬리피지는 반영하지 않으며 결과 화면에 이 한계를 함께 표시합니다.
