...
- 기본 30일과 90일·1년·전체 기간, 시작일·종료일 직접 입력을 지원합니다. 지수 데이터는 SQLite에 캐시합니다.
...
- SQLite에는 읽어 온 체결 주문, 종목 메타데이터, 종목별 일봉 OHLC, 비교 지수 종가, 백필 진행 상태, 날짜별 평가액과 계산된 비중을 저장합니다. 토스 자격증명과 액세스 토큰은 저장하지 않습니다.
