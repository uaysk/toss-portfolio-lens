...
```mermaid
flowchart LR
    Browser[Web UI] --> Node[Node.js control plane]
    MCP[MCP client] --> Node
    Node --> Toss[Toss API / compatible API]
    Node --> DB[(SQLite / PostgreSQL / MySQL)]
    Node -->|persistent UDS| Rust[Rust compute worker]
    Node -->|durable job| Queue[(PostgreSQL queue)]
    Queue --> RustExternal[Rust external worker]
    Rust --> Node
    RustExternal --> Queue
```
