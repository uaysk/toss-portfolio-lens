...
- 첫 전환에서는 스냅샷·종목 비중·체결 주문·종목 정보·종목/지수 일봉·환율·백필 상태를 트랜잭션으로 upsert합니다. SQLite보다 최신인 MySQL 행은 덮어쓰지 않습니다.
...
- KOSPI·KOSDAQ 공식 지수 일봉과 나스닥 100(QQQ)·S&P 500(SPY) 조정주가 프록시를 시작점 대비 등락률로 같은 화면에서 비교합니다.
...
- 선택된 SQLite 또는 MySQL에는 읽어 온 체결 주문, 종목 메타데이터, 종목별 일봉 OHLC, 비교 지수 종가, 환율, 백필 진행 상태, 날짜별 평가액과 계산된 비중을 저장합니다. 토스 자격증명과 액세스 토큰은 저장하지 않습니다.
