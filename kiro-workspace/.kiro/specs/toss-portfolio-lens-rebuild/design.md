...
flowchart LR
    ...
    S --> SQ[(SQLite)]
    S --> MY[(MySQL)]
    R --> OAI[OpenAI Responses compatible API]
...
5. 비용·세금·slippage는 0으로 두고 limitations에 기록한다.
6. portfolio value, cumulative contribution, benchmark normalized value, drawdown을 일별로 저장한다.
...
4. growth and contribution chart
5. drawdown, annual returns, contributions, correlation matrix
6. report 생성과 limitations
...
- mock Responses API의 success/refusal/429/invalid JSON
